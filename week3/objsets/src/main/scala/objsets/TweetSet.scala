...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = right.mostRetweetedAcc(left.mostRetweetedAcc(max(acc, elem)))

  def max(t1: Tweet, t2: Tweet): Tweet = if(t1.retweets >= t2.retweets) t1 else t2

  def descendingByRetweet: TweetList = {
    val top1 = mostRetweeted;
    new Cons(top1, remove(top1).descendingByRetweet )
  }
